...
export async function run() {
  ...
  assert.match(chatPage, /const cockpitResumeActions =\s+composerFollowUpActions.length > 0 \|\| starterPromptActions.length > 0\s+\? \[]\s+: resumeActions;/, 'cockpit continuation chips should stand down when composer follow-up or quick-start actions are present');
