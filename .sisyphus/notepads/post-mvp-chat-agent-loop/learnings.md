...
- 2026-03-09 (Iteration 26): “默认选中第一个 runnable agent”还不够；对于持续 coding 场景，更重要的是“恢复上次工作对象”。只要 localStorage 中记住的是一个仍可运行的 agent，用户就应该直接回到那个上下文。
...
- 2026-03-09 (Iteration 38): 真正决定 retry 是否可见的，不是“上一条 assistant 还在不在”，而是“当前失败的 turn 是否还有足够上下文可以重新发起”。在这个页面里，`lastUserMessage + selectedAgentRunnable` 比 `lastAssistantMessage` 更可靠。
- 2026-03-09 (Iteration 38): 真实浏览器回放已经证明新的 retry 条不是摆设：第一次 regenerate 失败后，composer 附近出现 `Retry last answer`，点击后 `chatCallCount` 从 2 增长到 3，说明它真的再次触发了请求，而不只是显示一个安慰性按钮。
